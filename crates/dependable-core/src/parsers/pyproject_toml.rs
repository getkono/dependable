    ...
    fn parse(&self, content: &str) -> Result<ParsedManifest, ParseError> {
            ...
            nav(root, &["tool", "poetry", "group"]).and_then(TomlItem::as_table_like)
        {
            for (_name, group) in groups.iter() {
                ...
                {
                    // Any Poetry group is a development grouping (`dev`, `test`, `docs`).
                    collect_table_deps(deps, DependencyKind::Dev, &starts, &mut items);
                }
            }
        }
        if let Some(t) =
        ...
        {
            collect_table_deps(t, DependencyKind::Dev, &starts, &mut items);
        }
        ...
        if let Some(arr) = nav(root, &["project", "dependencies"]).and_then(TomlItem::as_array) {
            collect_pep508_array(arr, DependencyKind::Normal, &starts, &mut items);
        }
        if let Some(t) =
            nav(root, &["project", "optional-dependencies"]).and_then(TomlItem::as_table_like)
        {
            for (_group, value) in t.iter() {
                if let Some(arr) = value.as_array() {
                    // An extra is installed only when the consumer asks for it.
                    collect_pep508_array(arr, DependencyKind::Optional, &starts, &mut items);
                }
            }
        }
            ...
            .and_then(TomlItem::as_table_like)
        {
            for (_group, value) in t.iter() {
                if let Some(arr) = value.as_array() {
                    // PEP 735 groups exist for development workflows, never for install.
...
fn collect_table_deps(
    ...
    for (name, item) in table.iter() {
        ...
        if let Some(parsed) = parse_table_dep(name, item, kind, starts) {
            items.push(parsed);
        }
    }
}
...
mod tests {
    ...
    fn section_determines_dependency_kind() {
        ...
        let m = parse(content);
        assert_eq!(find(&m, "flask").kind, DependencyKind::Normal);
        assert_eq!(find(&m, "weasyprint").kind, DependencyKind::Optional);
        assert_eq!(find(&m, "pytest").kind, DependencyKind::Dev);
        assert_eq!(find(&m, "black").kind, DependencyKind::Dev);
    }
